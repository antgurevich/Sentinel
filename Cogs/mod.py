...
###########################################################################
    @commands.command(name="mutelist", aliases=["listmutes"])
    @commands.has_permissions(kick_members=True)
    async def mutelist(self, ctx, user: discord.Member):
        cursor,conn=dbconnect() #Opens connection to db
        try:
            sql=('''SELECT row_id, duration, reason FROM moderator_history WHERE (guild_id=%s AND user_id=%s AND type=%s);''')
            cursor.execute(sql, (ctx.guild.id, user.id, "mute"))
            results=cursor.fetchall()
        except Exception as e:
            print (e)
        if len(results)==0:
            await ctx.send(embed=discord.Embed(title="This user has no history of mutes!"))
            return
        embed=discord.Embed(title=f"{user}'s Mute History:",color=discord.Color.dark_blue())
        x=0
        for mute in results:
            embed.add_field(name=f"{x+1})  Mute ID: {results[x][0]}",value=f"Reason: {results[x][2]} --- Duration: {results[x][1]}")
            x+=1
        await ctx.send(embed=embed)
        conn.close() #Closes connection to db
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send(embed=discord.Embed(title="Incorrect argument type! Use `.s mutelist (@user)`"))
        elif isinstance(error, commands.MissingRequiredArgument):
            await ctx.send(embed=discord.Embed(title="Enter the person you want to list the warns of! Use `.s mutelist (@user)`"))
###########################################################################
    @commands.command(name="clearmute")
    @commands.has_permissions(kick_members=True)
    async def clearmute(self, ctx, user: discord.Member, id: int):       
        cursor,conn=dbconnect() #Opens connection to db      
        try:
            sql=('''SELECT * FROM moderator_history WHERE (guild_id=%s AND user_id=%s AND row_id=%s);''')
            cursor.execute(sql, (ctx.guild.id, user.id,id))
            results=cursor.fetchone()
            if results is None:
                await ctx.send(embed=discord.Embed(title="Mute ID not found! Use `.s mutelist (@user)` to select the right ID!"))
                return
            ...
            print (e)
        await ctx.send(embed=discord.Embed(title="Mute successfully deleted!"))
        conn.close() #Closes connection to db
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send(embed=discord.Embed(title="Incorrect argument type! Use `.s clearmute (@user) (mute ID)`"))
        elif isinstance(error, commands.MissingRequiredArgument):
            await ctx.send(embed=discord.Embed(title="Enter the person & mute ID you want to clear the mute history from!\nUse `.s mutelist (@user)` to find the IDs, and then `.s clearmute (@user) (mute ID)`"))
###########################################################################
